...
Public Class BankCash_G2
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        If lop Then Return
        Try
            If Md.MyProjectType = ProjectType.X3 AndAlso e.RowIndex < G.Rows.Count - 1 AndAlso Val(G.Rows(e.RowIndex).Cells(GC.DocNo).Value) = 0 Then
    ...
    Sub ClearControls()
        ...
        If Md.MyProjectType = ProjectType.X2 AndAlso Not DayDate.SelectedDate Is Nothing Then
    ...
    Private Sub CalcBalSub(i As Integer)
        If Not lop AndAlso Val(G1.Rows(i).Cells(GC.SubAccNo).Value) > 0 Then
            CurrentBal.Content = bm.ExecuteScalar("select dbo.Bal0Link('" & G1.Rows(i).Cells(GC.LinkFile).Value & "','" & G1.Rows(i).Cells(GC.SubAccNo).Value & "','" & bm.ToStrDate(bm.MyGetDate()) & "',0)")
        Else
            CurrentBal.Content = ""
        End If
    End Sub
    ...
    Private Sub SetGCExchange(G As MyGrid)
        If Md.MyProjectType = ProjectType.X2 Then Return
        Try
            If Flag = 2 Then
