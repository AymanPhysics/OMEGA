...
Imports ToastNotifications.Messages
...
Public Class BasicMethods
    ...
    Public Path As String = System.Windows.Forms.Application.StartupPath & "\Temp\"



    Public Sub ToastShowInformation(Title As String)
        CType(Application.Current.MainWindow, MyMainWindow).notifier.ShowInformation(Title)
    End Sub
    Public Sub ToastShowError(Title As String)
        CType(Application.Current.MainWindow, MyMainWindow).notifier.ShowError(Title)
    End Sub
    Public Sub ToastShowSuccess(Title As String)
        CType(Application.Current.MainWindow, MyMainWindow).notifier.ShowSuccess(Title)
    End Sub
    Public Sub ToastShowWarning(Title As String)
        CType(Application.Current.MainWindow, MyMainWindow).notifier.ShowWarning(Title)
    End Sub


    Public Sub Alert(ByVal msg As String, ByVal type As Form_Alert.enmType, MyWindowToBeShown As UserControl)
        Dim frm As Form_Alert = New Form_Alert()
        frm.showAlert(msg, type, MyWindowToBeShown)
    End Sub
    ...
    Public Function GetAltScreenshot(Optional C As Grid = Nothing) As Integer
        Try
            If Not C Is Nothing Then
                ...
                graphics.CopyFromScreen(30, 120, 0, 0, New Size(printscreen.Size.Width, printscreen.Size.Height))
    ...
    Public Sub TextToSpeech(_Str As String)
        ...
        Try
            Dim WMP As WMPLib.WindowsMediaPlayer = CType(Application.Current.MainWindow, MyMainWindow).WMP
            Dim web As New System.Net.WebClient()
            web.Headers.Add(System.Net.HttpRequestHeader.UserAgent, "Mozilla/4.0 (compatible MSIE 9.0 Windows)")
            Dim encstr As String
            Dim x As Integer = 0
    ...
    Public Function ShowHelpMultiColumnsSales(ByVal Header As String, ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, ByVal Statement As String) As Boolean
        If e Is Nothing OrElse e.Key = System.Windows.Input.Key.F1 Then
            Dim hh As New HelpMultiColumnsSales
            SelectedRow = Nothing
            hh.Header = Application.Current.MainWindow.Resources.Item(Header)
            If hh.Header = "" Then hh.Header = Header
            hh.Statement = Statement
    ...
    Public Function ShowHelpGridItemBal(ByVal Column1 As Forms.DataGridViewCell, ByVal Column2 As Forms.DataGridViewCell, ByVal e As Forms.KeyEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "") As Boolean
        If Md.MyProjectType = ProjectType.Clothes AndAlso e.KeyCode = Forms.Keys.F12 Then
            Dim hh As New Help
            hh.Header = "الأرصدة"
            hh.Statement = Statement
            hh.TableName = TableName
    ...
    Public Function ShowHelpCases(ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, Optional ExistsOnly As Boolean = False, Optional AllowAdd As Boolean = True, Optional ForceAllowAdd As Boolean = False) As Boolean
        If Md.MyProjectType = ProjectType.APS Then AllowAdd = False
...
A:
    ...
    Public Sub SendEMail2(ByVal Bath As String)
        ...
        mailMsg.From = New MailAddress(Md.gmail)
...
A:
    ...
    Sub AddKeyDownToForm(frm As Window)
        Dim main As MyMainWindow = Application.Current.MainWindow
    ...
    Public Function SendWhatsApp(phone As String, Optional fileName As String = Nothing) As Boolean
        If phone.Length = 10 AndAlso phone.Substring(0, 1) = 1 Then
            phone = "20" & phone
        ElseIf phone.Length = 11 AndAlso phone.Substring(0, 2) = "01" Then
            phone = "2" & phone
        End If

        Dim DOCUMENT_SINGLE_API_URL As String = "http://api.whatsmate.net/v3/whatsapp/single/document/message/" & Md.whatsmateINSTANCE_ID

        Dim bytes As Byte() = File.ReadAllBytes(fileName)
        Dim base64Content As String = Convert.ToBase64String(bytes)
        Dim fn As String = "YourID.pdf"

        Dim success As Boolean = True
        Try
            Using client As New WebClient()
                client.Headers(HttpRequestHeader.ContentType) = "application/json"
                client.Headers("X-WM-CLIENT-ID") = Md.whatsmateCLIENT_ID
                client.Headers("X-WM-CLIENT-SECRET") = Md.whatsmateCLIENT_SECRET

                Dim payloadObj As New SingleDocPayload() With {.number = phone, .document = base64Content, .filename = fn}
                Dim postData As String = (New JavaScriptSerializer()).Serialize(payloadObj)

                client.Encoding = Encoding.UTF8
                Dim Response As String = client.UploadString(DOCUMENT_SINGLE_API_URL, postData)
                'ShowMSG(Response)
            End Using
        Catch webEx As WebException
            'ShowMSG(CType(webEx.Response, HttpWebResponse).StatusCode)
            Dim Stream As Stream = CType(webEx.Response, HttpWebResponse).GetResponseStream()
    ...
    Public Function SendSmsMisr(phone As String, SMS As String) As Boolean
        If phone.Length = 10 AndAlso phone.Substring(0, 1) = 1 Then
            phone = "20" & phone
        ElseIf phone.Length = 11 AndAlso phone.Substring(0, 2) = "01" Then
            phone = "2" & phone
        ...
        Dim API_URL As String = "https://smsmisr.com/api/webapi/?username=" & Md.smsmisrUsername & "&password=" & Md.smsmisrPassword & "&language=2&sender=" & Md.smsmisrSenderID & "&mobile=" & phone & "&message=" & SMS
        ...
        Try
            Using client As New WebClient()
                client.Headers(HttpRequestHeader.ContentType) = "application/json"
                client.Encoding = Encoding.UTF8
                Dim Response As String = client.UploadString(API_URL, "")
            End Using
        Catch webEx As WebException
            Dim Stream As Stream = CType(webEx.Response, HttpWebResponse).GetResponseStream()
            Dim reader As New StreamReader(Stream)
    ...
    Public Function SendWhatsAppOLD(phone As String, body As String, Optional fileName As String = Nothing) As Boolean
        Try
