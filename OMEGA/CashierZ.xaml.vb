...
Public Class CashierZ
    ...
    Private Sub DG_MouseDoubleClick(sender As Object, e As MouseButtonEventArgs) Handles DG.MouseDoubleClick
        ...
        If bm.ShowDeleteMSG("هل أنت متأكد من استلام الحافظة من الموظف " & DG.SelectedItems(0)(1) & " بمبلغ " & DG.SelectedItems(0)(4)) Then ', Value
            If bm.ExecuteNonQuery("update EmpCloseShift set SaveId='" & Val(SaveId.Text) & "',SaveGetDate=getdate(),SaveDate=CAST(GETDATE() AS DATE),SaveUsername=" & Md.UserName & " where  EmpId='" & DG.SelectedItems(0)(0) & "' and CurrentShift='" & DG.SelectedItems(0)(2) & "'  ") Then
                PrintPone(DG.SelectedItems(0)(0), DG.SelectedItems(0)(4))
    ...
    Private Sub PrintPone(InvoiceNo As Integer, Payed As String)
        ...
        rpt.paraname = New String() {"@FromDate", "@ToDate", "@Shift", "@Flag", "@FromInvoiceNo", "@ToInvoiceNo", "@InvoiceNo", "@NewItemsOnly", "@RPTFlag1", "@RPTFlag2", "@PrintingGroupId", "Header", "Remaining", "Payed", "@GroupId", "@TypeId", "@ItemId", "@Mobile", "@CarNo", "Bal"}
        rpt.paravalue = New String() {DayDate.SelectedDate, DayDate.SelectedDate, 0, 13, InvoiceNo, InvoiceNo, InvoiceNo, 0, 0, 0, 0, CType(Parent, Page).Title, 0, Val(Payed), 0, 0, 0, "", "", DG.SelectedItems(0)(4)}
