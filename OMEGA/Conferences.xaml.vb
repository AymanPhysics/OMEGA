...
Public Class Conferences
    ...
    Private Sub BtnPrintId_Click(sender As Object, e As RoutedEventArgs) Handles btnPrintId.Click
        ...
        DontClear = False
        Print(ConferenceId.Text, CustomerId.Text, CustomerName.Text)

        btnNew_Click(sender, e)

    End Sub

    Public Sub Print(_ConferenceId As String, _CustomerId As String, _CustomerName As String)
        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"CustomerId", "CustomerName"}
        rpt.paravalue = New String() {_CustomerId, _CustomerName}
        rpt.Rpt = "Id.rpt"
        ...
        'rpt.Show()
        rpt.Print()
        bm.ExecuteNonQuery("insert PrintIDsHistory(ConferenceId,CustomerId) select '" & _ConferenceId & "','" & _CustomerId & "'")
    End Sub


    Public Function PDF(_ConferenceId As String, _CustomerId As String, _CustomerName As String, Optional EmailReceivers() As String = Nothing, Optional WhatsAppReceiver As String = Nothing) As String
        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"CustomerId", "CustomerName"}
        rpt.paravalue = New String() {_CustomerId, _CustomerName}
        rpt.Rpt = "Id.rpt"
        ...
        Next
        bm.ClearTemp()
        If Not IO.Directory.Exists(bm.Path) Then IO.Directory.CreateDirectory(bm.Path)
        Dim fileName As String = bm.Path & _CustomerId & ".pdf"
        Try
            rpt.ReportDoc.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, fileName)
        Catch ex As Exception
        End Try
        rpt.ReportDoc.Dispose()
        rpt.Dispose()
        If Not EmailReceivers Is Nothing Then
            bm.SendEMail(CompanyName, "<H1>" & CompanyName & "</H1> <br/> <H3>Your ID is  <br/> " & _CustomerId & "</H3>", fileName, EmailReceivers)
        End If
        If Not WhatsAppReceiver Is Nothing Then
            bm.SendWhatsApp(WhatsAppReceiver, fileName)
        End If
        Return fileName
    End Function
    ...
    Private Sub CustomerId_KeyUp(sender As Object, e As KeyEventArgs) Handles CustomerId.KeyUp, MobileNumber.KeyUp, WhatsappNumber.KeyUp, InvoiceNo.KeyUp
        If bm.ShowHelpMultiColumns("Customers", CustomerId, CustomerName, e, "select cast(C.Id as varchar(100)) 'Id',C.Name,C.MobileNumber,C.WhatsappNumber,C.Code,S.Name SponsorName from Customers C left join Sponsors S on(C.SponsorId=S.Id)") Then
    ...
    Private Sub btnImportFromExcel_Click(sender As Object, e As RoutedEventArgs) Handles btnImportFromExcel.Click
        ...
        'Sponser
        Dim DistinctDt As DataTable = MyView.ToTable(True, dtAll.Columns(4).ColumnName)
        ...
        'Country
        DistinctDt = MyView.ToTable(True, dtAll.Columns(5).ColumnName)
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from Countries where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from Countries) insert Countries(Id,Name)select @Id,'" & DistinctDt.Rows(i)(0).ToString.Trim & "' end")
        Next


        'Cities
        DistinctDt = MyView.ToTable(True, {dtAll.Columns(5).ColumnName, dtAll.Columns(6).ColumnName})
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from Cities where Name='" & DistinctDt.Rows(i)(1).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from Cities) insert Cities(CountryId,Id,Name)select (Select Id from Countries where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "'),@Id,'" & DistinctDt.Rows(i)(1).ToString.Trim & "' end")
        Next


        'Universities
        DistinctDt = MyView.ToTable(True, dtAll.Columns(7).ColumnName)
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from Universities where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from Universities) insert Universities(Id,Name)select @Id,'" & DistinctDt.Rows(i)(0).ToString.Trim & "' end")
        Next

        'Specialties
        DistinctDt = MyView.ToTable(True, dtAll.Columns(9).ColumnName)
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from Specialties where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from Specialties) insert Specialties(Id,Name)select @Id,'" & DistinctDt.Rows(i)(0).ToString.Trim & "' end")
        Next

        'SubSpecialties
        DistinctDt = MyView.ToTable(True, {dtAll.Columns(9).ColumnName, dtAll.Columns(10).ColumnName})
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from SubSpecialties where Name='" & DistinctDt.Rows(i)(1).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from SubSpecialties) insert SubSpecialties(SpecialtyId,Id,Name)select (Select Id from Specialties where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "'),@Id,'" & DistinctDt.Rows(i)(1).ToString.Trim & "' end")
        Next

        'Degrees
        DistinctDt = MyView.ToTable(True, dtAll.Columns(11).ColumnName)
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from Degrees where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from Degrees) insert Degrees(Id,Name)select @Id,'" & DistinctDt.Rows(i)(0).ToString.Trim & "' end")
        Next

        'Titles
        DistinctDt = MyView.ToTable(True, dtAll.Columns(12).ColumnName)
        For i As Integer = 0 To DistinctDt.Rows.Count - 1
            If DistinctDt.Rows(i)(0).ToString.Trim = "" Then Continue For
            bm.ExecuteNonQuery("if not exists(select Id from Titles where Name='" & DistinctDt.Rows(i)(0).ToString.Trim & "') begin  declare @Id int=(select isnull(max(Id),0)+1 from Titles) insert Titles(Id,Name)select @Id,'" & DistinctDt.Rows(i)(0).ToString.Trim & "' end")
        Next
        ...
        For i As Integer = 0 To dtAll.Rows.Count - 1
            ...
            Dim MyMobileNumber As String = dtAll.Rows(i)(2).ToString.Trim.Replace("'", "''").Replace(" ", "''").Replace("-", "''")

            sql &= "  select @Id=null,@Code=null   select @Id=Id,@Code=Code from Customers where ('" & MyMobileNumber & "'<>'' and '" & MyMobileNumber & "' in(MobileNumber,WhatsappNumber))       if @Code is null                                                                                                                                                                                                                 begin  select @Code =(select isnull(MAX(Code),10000)+1 from Customers)  select @Id =dbo.ean13(@Code) insert Customers(Id,Name,MobileNumber,WhatsappNumber,Email,code,SponsorId,CountryId,CityId,UniversityId,WorkPlace,SpecialtyId,SubSpecialtyId,DegreeId,TitleId) select @Id,'" & dtAll.Rows(i)(1).ToString.Trim.Replace("'", "''") & "','" & dtAll.Rows(i)(2).ToString.Trim.Replace("'", "''") & "','" & dtAll.Rows(i)(2).ToString.Trim.Replace("'", "''") & "','" & dtAll.Rows(i)(3).ToString.Trim.Replace("'", "''") & "',@Code,(select Id from Sponsors where Name='" & dtAll.Rows(i)(4).ToString.Trim.Replace("'", "''") & "'),(select Id from Countries where Name='" & dtAll.Rows(i)(5).ToString.Trim.Replace("'", "''") & "'),(select Id from Cities where Name='" & dtAll.Rows(i)(6).ToString.Trim.Replace("'", "''") & "'),(select Id from Universities where Name='" & dtAll.Rows(i)(7).ToString.Trim.Replace("'", "''") & "'),'" & dtAll.Rows(i)(8).ToString.Trim.Replace("'", "''") & "',(select Id from Specialties where Name='" & dtAll.Rows(i)(9).ToString.Trim.Replace("'", "''") & "'),(select Id from SubSpecialties where Name='" & dtAll.Rows(i)(10).ToString.Trim.Replace("'", "''") & "'),(select Id from Degrees where Name='" & dtAll.Rows(i)(11).ToString.Trim.Replace("'", "''") & "'),(select Id from Titles where Name='" & dtAll.Rows(i)(12).ToString.Trim.Replace("'", "''") & "')  end                                                                                                                                                                                                  else                                                                                                                                            begin  update Customers set Name='" & dtAll.Rows(i)(1).ToString.Trim.Replace("'", "''") & "',MobileNumber='" & dtAll.Rows(i)(2).ToString.Trim.Replace("'", "''") & "',WhatsappNumber='" & dtAll.Rows(i)(2).ToString.Trim.Replace("'", "''") & "',Email='" & dtAll.Rows(i)(3).ToString.Trim.Replace("'", "''") & "',SponsorId=(select Id from Sponsors where Name='" & dtAll.Rows(i)(4).ToString.Trim.Replace("'", "''") & "'),CountryId=(select Id from Countries where Name='" & dtAll.Rows(i)(5).ToString.Trim.Replace("'", "''") & "'),CityId=(select Id from Cities where Name='" & dtAll.Rows(i)(6).ToString.Trim.Replace("'", "''") & "'),UniversityId=(select Id from Universities where Name='" & dtAll.Rows(i)(7).ToString.Trim.Replace("'", "''") & "'),WorkPlace='" & dtAll.Rows(i)(8).ToString.Trim.Replace("'", "''") & "',SpecialtyId=(select Id from Specialties where Name='" & dtAll.Rows(i)(9).ToString.Trim.Replace("'", "''") & "'),SubSpecialtyId=(select Id from SubSpecialties where Name='" & dtAll.Rows(i)(10).ToString.Trim.Replace("'", "''") & "'),DegreeId=(select Id from Degrees where Name='" & dtAll.Rows(i)(11).ToString.Trim.Replace("'", "''") & "'),TitleId=(select Id from Titles where Name='" & dtAll.Rows(i)(12).ToString.Trim.Replace("'", "''") & "') where Id=@Id   end                                                                                                                                                insert ConferencesMaster(ConferenceId,InvoiceNo,CustomerId,Conference,SponsorId) select '" & Val(ConferenceId.Text) & "',@Id,@Id,1,(select Id from Sponsors where Name='" & dtAll.Rows(i)(4).ToString.Trim.Replace("'", "''") & "') where not exists(select ConferenceId from ConferencesMaster where ConferenceId='" & Val(ConferenceId.Text) & "' and InvoiceNo=@Id ) "
