...
Public Class Inventory
    ...
    Private Sub LoadTypes(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            If All.IsChecked Then
                Dim str As String = "select It.Id,isnull(sum(Qty),0)Qty from Items It left join dbo.Fn_AllItemMotion(" & StoreId.Text & ",0,'" & bm.ToStrDate(DayDate.SelectedDate) & "')Fn on(It.Id=Fn.ItemId) where It.GroupId=" & xx.Tag & " and IsService=0 and IsStopped=0 group by It.Id"


                Dim dtdt As DataTable = bm.ExecuteAdapter(str)
                For i As Integer = 0 To dtdt.Rows.Count - 1
                    If Not IsHasbal.IsChecked Or Val(dtdt.Rows(i)("Qty")) <> 0 Then
    ...
    Private Sub LoadItems(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            If All.IsChecked Then
                Dim str As String = "select It.Id,isnull(sum(Qty),0)Qty from Items It left join dbo.Fn_AllItemMotion(" & StoreId.Text & ",0,'" & bm.ToStrDate(DayDate.SelectedDate) & "')Fn on(It.Id=Fn.ItemId) where It.GroupId=" & WTypes.Tag.ToString & " and It.TypeId=" & xx.Tag & " and IsService=0 and IsStopped=0 group by It.Id"
