...
Public Class Inventory2
    ...
    Public TableName As String = "Inventory2Master"
    Public TableDetailsName As String = "Inventory2Details"
    ...
    Private Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.Fields = New String() {MainId, SubId, "DayDate", "Notes", "DocNo"}
        bm.control = New Control() {StoreId, InvoiceNo, DayDate, Notes, DocNo}
...
Br:
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        Try
            ...
            G.Rows(e.RowIndex).Cells(GC.CurrentBal).Value = bm.ExecuteScalar("select dbo.FnStoreIetmBal(" & Val(StoreId.Text) & "," & G.Rows(e.RowIndex).Cells(GC.Id).Value & ",'',0,0," & Val(G.Rows(e.RowIndex).Cells(GC.UnitId).Value) & ",'" & bm.ToStrDate(DayDate.SelectedDate) & "')")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If InvoiceNo.Text.Trim = "" Then
            InvoiceNo.Text = bm.ExecuteScalar("select isnull(max(" & SubId & "),0)+1 from " & TableName & " where " & MainId & "='" & StoreId.Text & "'")
        ...
        If Not bm.SaveGrid(G, TableDetailsName, New String() {"StoreId", "InvoiceNo"}, New String() {StoreId.Text, InvoiceNo.Text}, New String() {"Barcode", "ItemId", "ItemName", "UnitId", "UnitQty", "CurrentBal", "Qty"}, New String() {GC.Barcode, GC.Id, GC.Name, GC.UnitId, GC.UnitQty, GC.CurrentBal, GC.Qty}, New VariantType() {VariantType.String, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal}, New String() {GC.Id}, "Line", "Line") Then Return
    ...
    Private Sub btnPrint_Copy_Click(sender As Object, e As RoutedEventArgs) Handles btnPrint.Click
        ...
        If e.Key = Key.Tab Then
            ...
            For i As Integer = 0 To G.Rows.Count - 1
                If G.Rows(i).Cells(GC.Barcode).Value = Barcode.Text Then
                    G.Rows(i).Cells(GC.Qty).Value = Val(G.Rows(i).Cells(GC.Qty).Value) + 1
                    Barcode.Clear()
                    G.CurrentCell = G.Rows(i).Cells(GC.Name)
                    GridCalcRow(Nothing, New Forms.DataGridViewCellEventArgs(G.Columns(GC.Qty).Index, i))
                    IsExists = True
                    Exit For
                End If
            Next
            ...
            If Not IsExists Then
                ...
                G.CurrentRow.Cells(GC.Qty).Value = Val(G.CurrentRow.Cells(GC.Qty).Value) + 1
