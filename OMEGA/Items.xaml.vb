...
Public Class Items
    ...
    Private Sub BasicForm_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.FillCombo("Boards", BoardId, "", , True)
        bm.FillCombo("SubjectLevels", SubjectLevelId, "", , True)
        bm.FillCombo("Subjects", SubjectId, "", , True)
        ...
        If Not Md.MyProjectType = ProjectType.ClinicPublicElSadaka Then
        ...
        If Md.MyProjectType = ProjectType.X3 OrElse Md.MyProjectType = ProjectType.AsebAcc Then
        ...
        If Not Md.MyProjectType = ProjectType.RivieraCenter Then
            ...
            lblBoardId.Visibility = Visibility.Hidden
            SubjectLevelId.Visibility = Visibility.Hidden
            lblSubjectLevelId.Visibility = Visibility.Hidden
            SubjectId.Visibility = Visibility.Hidden
            lblSubjectId.Visibility = Visibility.Hidden
            TeacherId.Visibility = Visibility.Hidden
            TeacherName.Visibility = Visibility.Hidden
            lblTeacherId.Visibility = Visibility.Hidden
        End If

        If Not Md.ShowPriceLists Then
            TabTypes1.Visibility = Visibility.Collapsed
        End If
    ...
    Private Sub TeacherId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles TeacherId.KeyUp
        If bm.ShowHelp("Teachers", TeacherId, TeacherName, e, "select cast(Id as varchar(100)) Id,Name from Teachers") Then
    ...
    Private Sub LoadWFH2()
        ...
        G2.Columns.Add(GC2.AccNo, "كود الحساب")
        G2.Columns.Add(GC2.AccName, "اسم الحساب")
        G2.Columns.Add(GC2.rate, "rate %")

        G2.Columns(GC2.AccName).FillWeight = 250

        G2.Columns(GC2.AccName).ReadOnly = True
    ...
    Private Sub GridKeyDown(sender As Object, e As Forms.KeyEventArgs)
        ...
        If G2.CurrentCell.ColumnIndex = G2.Columns(GC2.AccNo).Index AndAlso bm.AccNoShowHelpGrid(G2.CurrentRow.Cells(GC2.AccNo), G2.CurrentRow.Cells(GC2.AccName), e, 1) Then
            G2.CurrentCell = G2.Rows(G2.CurrentCell.RowIndex).Cells(GC2.rate)
        End If
    End Sub

    Private Sub GridCalcRow(sender As Object, e As Forms.DataGridViewCellEventArgs)
        If G2.CurrentCell Is Nothing Then Return
        If e.ColumnIndex = G2.Columns(GC2.AccNo).Index Then
            bm.AccNoLostFocusGrid(G2.Rows(e.RowIndex).Cells(GC2.AccNo), G2.Rows(e.RowIndex).Cells(GC2.AccName))
        End If
        G2.Rows(G2.CurrentCell.RowIndex).Cells(GC2.rate).Value = Val(G2.Rows(G2.CurrentCell.RowIndex).Cells(GC2.rate).Value)
    End Sub
    ...
    Sub GetGrid2()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            G2.Rows.Add()
            G2.Rows(i).Cells(GC2.AccNo).Value = dt.Rows(i)("AccNo").ToString
            GridCalcRow(G2, New Forms.DataGridViewCellEventArgs(G2.Columns(GC2.AccNo).Index, i))
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Md.MyProjectType = ProjectType.X3 AndAlso ItemUnitId.SelectedIndex < 1 Then
            bm.ShowMSG("برجاء تحديد الوحدة")
            ItemUnitId.Focus()
            Return
        End If

        If Md.MyProjectType = ProjectType.X3 AndAlso Val(CountryId.Text) = 0 Then
        ...
        UpdateDate.Text = bm.ExecuteScalar("Select dbo.ToStrDateTime(GETDATE())")
        ...
        If Not bm.SaveGrid(G2, TableDetailsName2, New String() {"ItemId"}, New String() {txtID.Text}, New String() {"AccNo", "rate"}, New String() {GC2.AccNo, GC2.rate}, New VariantType() {VariantType.String, VariantType.Decimal}, New String() {GC2.AccNo}) Then Return
    ...
    Sub ClearControls()
        ...
        txtName.Clear()
        TaxApi_UnitTypeId.SelectedIndex = 1
    ...
    Private Sub TypeId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles TypeId.LostFocus
        ...
        If IsNew AndAlso AllowGenerateItemId Then
            Dim str As String = bm.ExecuteScalar("select isnull(max(" & SubId & "),0)+1 from " & TableName & " where GroupId=" & Val(GroupId.Text) & " and TypeId=" & Val(TypeId.Text))
    ...
    Private Sub ItemType_SelectionChanged(ByVal sender As System.Object, ByVal e As System.Windows.Controls.SelectionChangedEventArgs) Handles ItemType.SelectionChanged
        ...
        If ItemType.SelectedIndex = 3 AndAlso Md.MyProjectType = ProjectType.X3 Then
    ...
    Private Sub WFH_LayoutError(sender As Object, e As LayoutExceptionEventArgs) Handles WFH.LayoutError
