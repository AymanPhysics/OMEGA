...
Public Class RPT43
    ...
    Public Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Button2.Click, Button3.Click, Button4.Click
        ...
        rpt.paravalue = New String() {bm.ToStrDate(FromDate.SelectedDate), bm.ToStrDate(ToDate.SelectedDate), IIf(Ban.IsChecked, 1, 0), CType(Parent, Page).Title, PaymentDay.SelectedValue, IIf(IsDelayed.IsChecked, 1, 0), IIf(All.IsChecked, 1, 0), 3}
    ...
    Private Sub UserControl_Loaded(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles MyBase.Loaded
        ...
        Button2_Copy2.Visibility = Visibility.Collapsed
    ...
    Private Sub LoadGrid2()
        If G.CurrentRow.Cells(GC.Id).Value Is Nothing Then Return
        Dim dt As DataTable = bm.ExecuteAdapter("select * from CustomerNotes2 where CustomerId=" & G.CurrentRow.Cells(GC.ToId).Value.ToString & " order by MyGetDateTime")
    ...
    Private Sub Button2_Copy1_Click(sender As Object, e As RoutedEventArgs) Handles Button2_Copy1.Click
        ...
        If bm.ExecuteNonQuery("update InstallmentInvoicesDateils set DelayedDate=getdate(),IsDelayed=1,DayDate='" & bm.ToStrDate(DayDate.SelectedDate) & "',Notes='" & Notes.Text.Replace("'", "''") & "',Value='" & Val(Value.Text) & "' where Flag='" & G.CurrentRow.Cells(GC.Flag).Value & "' and StoreId='" & G.CurrentRow.Cells(GC.StoreId).Value & "' and ToId='" & G.CurrentRow.Cells(GC.ToId).Value & "' and InvoiceNo='" & G.CurrentRow.Cells(GC.InvoiceNo).Value & "' and Id='" & G.CurrentRow.Cells(GC.Id).Value & "'") Then
            ...
            If Not bm.SaveGrid(G2, "CustomerNotes2", New String() {"CustomerId"}, New String() {G.CurrentRow.Cells(GC.ToId).Value}, New String() {"Notes", "EmpName", "MyGetDateTime"}, New String() {GC2.Notes, GC2.EmpName, GC2.MyGetDateTime}, New VariantType() {VariantType.String, VariantType.String, VariantType.String, VariantType.String}, New String() {GC2.Notes}, "", "") Then Return
    ...
    Private Sub GetData()
        Dim dt As DataTable = bm.ExecuteAdapter("GetInstallmentInvoicesDateils", {"FromDate", "ToDate", "Ban", "PaymentDay", "IsDelayed", "All", "Flag"}, {bm.ToStrDate(FromDate.SelectedDate), bm.ToStrDate(ToDate.SelectedDate), IIf(Ban.IsChecked, 1, 0), PaymentDay.SelectedValue, IIf(IsDelayed.IsChecked, 1, 0), IIf(All.IsChecked, 1, 0), 3})
    ...
    Private Sub Hyperlink_RequestNavigate(sender As Object, e As RequestNavigateEventArgs)
        ...
        Process.Start(New ProcessStartInfo("tel:" & G.CurrentRow.Cells(GC.Mobile).Value.ToString))
        e.Handled = True
    End Sub
