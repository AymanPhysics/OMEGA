...
Public Class RPTFromIdToId
    ...
    Public Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Button2.Click
        ...
        ElseIf Flag = 8 OrElse Flag = 9 Then

            dt = bm.ExecuteAdapter("select distinct C.Id,C.Name,C.Email,C.WhatsappNumber from ConferencesMaster T left join Customers C on(T.CustomerId=C.Id) where T.ConferenceId=" & Val(ConferenceId.Text) & " and (T.SponsorId=" & Val(SponsorId.Text) & " or " & Val(SponsorId.Text) & "=0) and (C.Code>=" & Val(FromId.Text) & " or " & Val(FromId.Text) & "=0) and (C.Code<=" & Val(ToId.Text) & " or " & Val(ToId.Text) & "=0)")

            Dim frm As New Conferences
            For i As Integer = 0 To dt.Rows.Count - 1
                If Flag = 8 Then
                    frm.PDF(Val(ConferenceId.Text), Val(dt.Rows(i)(0)), dt.Rows(i)(1), {dt.Rows(i)(2)})
                ElseIf Flag = 9 Then
                    frm.PDF(Val(ConferenceId.Text), Val(dt.Rows(i)(0)), dt.Rows(i)(1), , Val(dt.Rows(i)(3)))
                End If
            Next
            bm.ShowMSG("Done")
            Return
    ...
    Private Sub UserControl_Loaded(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles MyBase.Loaded
        ...
        If Flag <> 1 AndAlso Flag <> 8 AndAlso Flag <> 9 Then
