...
Public Class Sales
    ...
    Public TableDetailsNameTaxableItems As String = "SalesDetailsTaxableItems"
    ...
    Public Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        If Md.MyProjectType = ProjectType.MarketWithShifts OrElse Md.MyProjectType = ProjectType.Market Then
            btnNew.Content &= " (F8)"
        ...
        End If
        If Flag = FlagState.مردودات_المبيعات OrElse Flag = FlagState.مردودات_مشتريات OrElse Flag = FlagState.أمر_توريد Then
            If Md.MyProjectType = ProjectType.AboHamza Then MyGrid.Background = New Media.SolidColorBrush(Colors.Red)
        End If

        If Not Md.MyProjectType = ProjectType.RivieraCenter Then
        ...
        If Not (Md.MyProjectType = ProjectType.X3 AndAlso Flag = FlagState.مشتريات) Then
        ...
        End If
        If Md.MyProjectType = ProjectType.X3 AndAlso Flag = FlagState.المبيعات Then
        ...
        End If
        If Md.MyProjectType <> ProjectType.Installments OrElse (Flag <> FlagState.المبيعات AndAlso Flag <> FlagState.مبيعات_لمستثمر) Then
    ...
    Private Sub LoadWFH()
        ...
        G.Columns.Add(GC.rate, "نسبة الضريبة")
        G.Columns.Add(GC.amount, "قيمة الضريبة")
        ...
        If Md.MyProjectType = ProjectType.X3 OrElse Md.MyProjectType = ProjectType.AboHamza Then
        ...
        G.Columns(GC.PurchasePrice).Visible = Md.ShowPurchasePrice AndAlso TestSalesAndReturn()
        ...
        If Md.MyProjectType = ProjectType.X3 AndAlso Flag = FlagState.تحويل_إلى_مخزن Then
        ...
        If Not (Md.MyProjectType = ProjectType.RivieraCenter OrElse Md.MyProjectType = ProjectType.AboHamza OrElse Md.MyProjectType = ProjectType.X2) Then
        ...
        If Not (Md.MyProjectType = ProjectType.X1 AndAlso TestImportOnly()) Then
        ...
        If Md.ShowWeightQty Then
            G.Columns(GC.PreQty).Visible = True
            G.Columns(GC.SalesPriceTypeId).Visible = True
        End If
        ...
        If Md.MyProjectType = ProjectType.X2 Then
            ...
            Next
        End If
        G.Columns(GC.rate).ReadOnly = True
        G.Columns(GC.amount).ReadOnly = True
        If Not Md.ShowTaxETA Then
            G.Columns(GC.rate).Visible = False
            G.Columns(GC.amount).Visible = False
        End If
    ...
    Function ReadOnlyState() As Boolean
        If Md.MyProjectType = ProjectType.X3 AndAlso (Flag = FlagState.تسوية_صرف OrElse Flag = FlagState.صرف) Then
            Return True
        ElseIf Md.Manager OrElse Md.EditPrices Then
            Return False
        ElseIf Md.MyProjectType = ProjectType.ContractRaggal OrElse Md.MyProjectType = ProjectType.Perfume Then
            Return False
    ...
    Sub LoadAllItems()
        Try
            HelpDt = bm.ExecuteAdapter("Select cast(Id as nvarchar(100))Id," & Resources.Item("CboName") & " Name," & PriceFieldName(GC.Price, StoreUnitId) & " Price From Items  where IsStopped=0 " & ItemWhere())
    ...
    Function ItemWhere() As String
        ...
        If Md.MyProjectType = ProjectType.X3 AndAlso TestSalesAndReturn() Then
    ...
    Private Sub LoadItems(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            Dim dt As DataTable = bm.ExecuteAdapter("Select * From Items_View  where IsStopped=0 " & ItemWhere() & " and GroupId=" & WTypes.Tag.ToString & " and TypeId=" & xx.Tag.ToString & " order by " & IIf(Md.MyProjectType = ProjectType.RivieraCenter, "Id", "Name"))
            For i As Integer = 0 To dt.Rows.Count - 1
                Dim x As New Button
                bm.SetStyle(x, 370)
                'bm.SetImage(x, CType(dt.Rows(i)("Image"), Byte()))
        ...
        Try
            ...
            If xx >= 0 AndAlso xx <> i Then
                If Md.MyProjectType = ProjectType.RivieraCenter Then
                    G.CurrentCell = G.Rows(xx).Cells(GC.Qty)
                    Return xx
                Else
                    bm.ShowMSG("تم تكرار هذا الصنف بالسطر رقم " + (xx + 1).ToString)
                    If Md.PreventItemDoublication AndAlso TestSalesAndReturn() Then
                        ClearRow(i)
                        Return i
                    End If
                End If
...
Br:
            ...
            If (Md.MyProjectType = ProjectType.X3 OrElse Md.MyProjectType = ProjectType.X4) AndAlso (Flag = FlagState.إضافة OrElse Flag = FlagState.مشتريات OrElse Flag = FlagState.الاستيراد OrElse Flag = FlagState.تحويل_إلى_مخزن OrElse Flag = FlagState.المبيعات) Then
                If i = 0 Then
                    Dim x As Integer = 1
                    Select Case Flag
                        Case FlagState.تحويل_إلى_مخزن
    ...
    Sub CalcRow(ByVal i As Integer)
        Try
            ...
            If Md.CheckItemBal AndAlso Not lop AndAlso Md.MyProjectType <> ProjectType.TaxETA AndAlso (Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_مشتريات OrElse Flag = FlagState.مردودات_مشتريات_خارجية OrElse Flag = FlagState.تحويل_إلى_مخزن OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.صرف OrElse Flag = FlagState.تسوية_صرف) AndAlso Val(G.Rows(i).Cells(GC.Qty).Value) > Val(G.Rows(i).Cells(GC.CurrentBal).Value) Then
            ...
            G.Rows(i).Cells(GC.PurchasePrice).Value = Val(G.Rows(i).Cells(GC.PurchasePrice).Value)
    ...
    Sub ClearRow(ByVal i As Integer)
        ...
        G.Rows(i).Cells(GC.PurchasePrice).Value = Nothing
        ...
        G.Rows(i).Cells(GC.rate).Value = Nothing
        G.Rows(i).Cells(GC.amount).Value = Nothing
    ...
    Private Sub RdoCash_Checked(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles RdoCash.Checked, RdoVisa.Checked, RdoCashVisa.Checked, RdoFuture.Checked, RdoCashFuture.Checked, RdoEmployees.Checked
        ...
        Try
            If (RdoCash.IsChecked OrElse RdoCashFuture.IsChecked OrElse RdoCashVisa.IsChecked) AndAlso (TestPurchaseAndReturn() OrElse TestSalesAndReturn() OrElse TestExportAndReturn()) Then
                If Not Md.MyProjectType = ProjectType.StoresOnly Then
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        Try
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.ItemSerialNo AndAlso Not G.Rows(e.RowIndex).Cells(GC.ItemSerialNo).Value Is Nothing AndAlso Not G.Rows(e.RowIndex).Cells(GC.ItemSerialNo).Value = "" Then
                ...
                If dt.Rows.Count > 0 Then
                    If Md.MyProjectType = ProjectType.Installments AndAlso TestPurchaseOnly() Then
    ...
    Private Sub ToId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles ToId.KeyUp
        ...
        ElseIf TestConsumablesAndReturn() Then
            If bm.ShowHelpCases(ToId, ToName, e, IIf(Flag = FlagState.المستهلكات OrElse Flag = FlagState.مردودات_المستهلكات, False, True), True) Then
                ToId_LostFocus(sender, Nothing)
            End If
        End If
    End Sub
    ...
    Private Sub ToId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles ToId.LostFocus
        ...
        ElseIf TestConsumablesAndReturn() Then
            ...
            ToId.ToolTip = ""
            ToName.ToolTip = ""
            dt = bm.ExecuteAdapter("select HomePhone,Mobile,Notes,Ban from Customers where Id=" & ToId.Text.Trim())
            If dt.Rows.Count > 0 Then
        ...
        If Md.MyProjectType = ProjectType.X3 And TestSalesAndReturn() And Not lop Then
        ...
        If TestSalesAndReturn() Then
            ToName.Background = IIf(Val(bm.ExecuteScalar("select Ban from Customers where Id=" & ToId.Text)) = 1, New SolidColorBrush(Colors.Red), ToId.Background)
        End If


        If Md.MyProjectType = ProjectType.AboHamza AndAlso (TestSalesAndReturn() OrElse TestPurchaseAndReturn()) Then
            CalcBalSub()
        ElseIf Md.MyProjectType = ProjectType.PremiumMedicalSupplies Then
            GetCurrentBal()
        End If
        ...
        If Md.MyProjectType = ProjectType.RivieraCenter AndAlso TestSalesOnly() Then
            CalcPoints
        End If
        ...
        If ((TestSalesAndReturn() OrElse TestExportAndReturn()) AndAlso Not ReservToId.IsChecked) OrElse ((TestPurchaseAndReturn()) AndAlso ReservToId.IsChecked) Then
            ...
            If dt.Rows.Count > 0 Then
                ...
                End If
            End If
        ElseIf ((TestPurchaseAndReturn()) AndAlso Not ReservToId.IsChecked) OrElse ((TestSalesAndReturn()) AndAlso ReservToId.IsChecked) Then

            If InvoiceNo.Text.Trim = "" Then
                If bm.ExecuteScalar("select Type from Suppliers where Id=" & ToId.Text.Trim()) = "1" Then
                    RdoCash.IsChecked = True
                Else
                    RdoFuture.IsChecked = True
                End If
        ...
        'ToName.ToolTip = New ToolTip() With {.Content = s, .IsOpen = True}
    ...
    Private Sub CashierId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles CashierId.KeyUp
        bm.ShowHelp("Employees", CashierId, CashierName, e, "select cast(Id as varchar(100)) Id," & Resources.Item("CboName") & " Name from Employees where Doctor=0 and Stopped=0" & IIf(TestConsumablesAndReturn() AndAlso Md.MyProjectType = ProjectType.ClinicPublicElSadaka, " and Nurse=1", " and Cashier=1"))
    End Sub
    ...
    Private Sub CashierId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles CashierId.LostFocus
        bm.LostFocus(CashierId, CashierName, "select " & Resources.Item("CboName") & " Name from Employees where Doctor=0 and Id=" & CashierId.Text.Trim() & " and Stopped=0 " & IIf(TestConsumablesAndReturn() AndAlso Md.MyProjectType = ProjectType.ClinicPublicElSadaka, " and Nurse=1", " and Cashier=1"))
    End Sub
    ...
    Sub FillControls()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            If TestSalesOnly() Then G.Rows(i).Cells(GC.CurrentBal).Value += G.Rows(i).Cells(GC.Qty).Value
            ...
            G.Rows(i).Cells(GC.rate).Value = dt.Rows(i)("rate").ToString
            G.Rows(i).Cells(GC.amount).Value = dt.Rows(i)("amount").ToString
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles btnPrint.Click, btnPrint2.Click, btnPrint3.Click, btnPrint4.Click, btnPrint5.Click, btnPrintImage.Click, btnReceiveSave.Click, btnPrintSafe.Click
        If Md.MyProjectType = ProjectType.PremiumMedicalSupplies Then DontClear = True
        If Md.MyProjectType = ProjectType.AboHamza Then DontClear = False
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Md.MyProjectType = ProjectType.AboHamza AndAlso TestSalesAndReturn() Then
            If bm.ShowDeleteMSG("هل تريد تغيير القسط الشهري؟") Then
                MonthlyPayment.SelectAll()
        ...
        If Md.MyProjectType = ProjectType.X3 AndAlso DocNo.Text.Trim = "" Then
        ...
        If btnSave.IsEnabled Then
            ...
            If Not bm.SaveGrid(G, "SalesDetails", New String() {"Flag", "StoreId", "InvoiceNo"}, New String() {Flag, StoreId.Text, InvoiceNo.Text}, New String() {"SalesInvoiceNo", "Barcode", "ItemSerialNo", "ItemId", "ItemName", "ExpDate", "ExpireDate", "Color", "Size", "UnitId", "UnitQty", "Qty", "Qty2", "Qty3", "ReceivedQty", "Price", "QtySub", "PriceSub", "ItemDiscountPerc", "ItemDiscount", "Value", "IsPrinted", "SalesPrice", "PurchasePrice", "FlagType", "SerialNo", "IsDelivered", "SalesManId", "ProductionOrderFlag", "ProductionOrderNo", "SD_Notes", "UnitsWeightId", "UnitsWeightQty", "PreQty", "SalesPriceTypeId", "AvgCost", "SalesReturnStateId", "SalesReturnStateReason", "ConsumptionQty", "ConsumptionRemainingQty", "rate", "amount"}, New String() {GC.SalesInvoiceNo, GC.Barcode, GC.ItemSerialNo, GC.Id, GC.Name, GC.ExpDate, GC.ExpireDate, GC.Color, GC.Size, GC.UnitId, GC.UnitQty, GC.Qty, GC.Qty2, GC.Qty3, GC.ReceivedQty, GC.Price, GC.QtySub, GC.PriceSub, GC.ItemDiscountPerc, GC.ItemDiscount, GC.Value, GC.IsPrinted, GC.SalesPrice, GC.PurchasePrice, GC.FlagType, GC.SerialNo, GC.IsDelivered, GC.SalesManId, GC.ProductionOrderFlag, GC.ProductionOrderNo, GC.SD_Notes, GC.UnitsWeightId, GC.UnitsWeightQty, GC.PreQty, GC.SalesPriceTypeId, GC.AvgCost, GC.SalesReturnStateId, GC.SalesReturnStateReason, GC.ConsumptionQty, GC.ConsumptionRemainingQty, GC.rate, GC.amount}, New VariantType() {VariantType.Integer, VariantType.String, VariantType.String, VariantType.Integer, VariantType.String, VariantType.String, VariantType.Date, VariantType.Integer, VariantType.Integer, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Integer, VariantType.Integer, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Integer, VariantType.Decimal, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Integer, VariantType.Decimal, VariantType.Decimal}, New String() {GC.Id}, "Line", "Line") Then Return
            ...
            If Md.MyProjectType = ProjectType.Perfume AndAlso TestSalesAndReturn() Then
...
Print:
        ...
        If sender Is btnPrint OrElse sender Is btnPrint2 OrElse sender Is btnPrint3 OrElse sender Is btnPrint4 OrElse sender Is btnPrint5 OrElse sender Is btnPrintImage OrElse sender Is btnPrintSafe OrElse (sender Is btnCloseTable And btnPrint.IsEnabled) Then
            PrintPone(sender, 0)
            If Md.MyProjectType = ProjectType.X3 AndAlso sender Is btnPrint AndAlso Flag = FlagState.المبيعات Then
    ...
    Sub ClearControls()
        Try
            ...
            If (Md.MyProjectType = ProjectType.RivieraCenter OrElse Md.MyProjectType = ProjectType.AboHamza) AndAlso TestSalesAndReturn() Then
            ...
            If Md.MyProjectType <> ProjectType.ClinicPublicElSadaka AndAlso Md.MyProjectType <> ProjectType.APSD AndAlso Md.MyProjectType <> ProjectType.APS AndAlso Md.MyProjectType <> ProjectType.X3 Then CashierId.Text = Md.UserName
            ...
            If TestPurchaseAndReturn() Then
                If Md.MyProjectType = ProjectType.PremiumMedicalSupplies AndAlso TestOuterPurchaseAndReturn() Then
                Else
            ...
            If Not Md.Manager Then
                ...
                If Not Md.MyProjectType = ProjectType.X3 Then CashierId.Text = Md.UserName
                CashierId_LostFocus(Nothing, Nothing)
            Else
        ...
        If TestSalesAndReturn() AndAlso Md.MyProjectType = ProjectType.X3 Then
    ...
    Private Sub InvoiceNo_KeyUp(sender As Object, e As KeyEventArgs) Handles InvoiceNo.KeyUp
        ...
        ElseIf TypeOf (Parent) Is Window Then
            title = CType(Parent, Window).Title
        End If

        If bm.ShowHelpMultiColumnsSales("Header", InvoiceNo, InvoiceNo, e, "exec getSalesMasterHelp " & Val(StoreId.Text) & "," & Flag.ToString) Then
            InvoiceNo_Leave(Nothing, Nothing)
        End If


        'If TestImportAndReturn() Then
        '    If bm.ShowHelpMultiColumnsSales("Header", InvoiceNo, InvoiceNo, e, "select cast(StoreId as nvarchar(100))'رقم المخزن',cast(Flag as nvarchar(100))'النوع',cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود المورد',dbo.GetSupplierName(ToId)'اسم المورد',dbo.GetOrderTypes(OrderTypeId)'رقم الطلبية',DocNo'رقم عقد المورد',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',cast(TotalAfterDiscount as nvarchar(100)) 'القيمة' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
        '        'InvoiceNo.Text = bm.SelectedRow(0)
        '        InvoiceNo_Leave(Nothing, Nothing)
        '    End If
        'ElseIf TestPurchaseAndReturn() Then
        '    If bm.ShowHelpMultiColumnsSales("Header", InvoiceNo, InvoiceNo, e, "select cast(StoreId as nvarchar(100))'رقم المخزن',cast(Flag as nvarchar(100))'النوع',cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود المورد',dbo.GetSupplierName(ToId)'رقم عقد المورد',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',cast(TotalAfterDiscount as nvarchar(100)) 'القيمة',case PaymentType when 1 then 'كاش' when 2 then 'فيزا' when 3 then 'كاش/فيزا' when 4 then 'آجل' when 5 then 'نقدي/آجل' else '-' end 'نوع الدفع' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
        '        'InvoiceNo.Text = bm.SelectedRow(0)
        '        InvoiceNo_Leave(Nothing, Nothing)
        '    End If
        'ElseIf TestSalesAndReturn() OrElse TestExportAndReturn() Then
        '    If bm.ShowHelpMultiColumnsSales(title, InvoiceNo, InvoiceNo, e, "select cast(StoreId as nvarchar(100))'رقم المخزن',cast(Flag as nvarchar(100))'النوع',cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود العميل',dbo.GetCustomerName(ToId)'اسم العميل',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',cast(TotalAfterDiscount as nvarchar(100)) 'القيمة',case PaymentType when 1 then 'كاش' when 2 then 'فيزا' when 3 then 'كاش/فيزا' when 4 then 'آجل' when 5 then 'نقدي/آجل' else '-' end 'نوع الدفع' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
        '        'InvoiceNo.Text = bm.SelectedRow(0)
        '        InvoiceNo_Leave(Nothing, Nothing)
        '    End If
        'Else
        '    If bm.ShowHelpMultiColumnsSales(title, InvoiceNo, InvoiceNo, e, "select cast(StoreId as nvarchar(100))'رقم المخزن',cast(Flag as nvarchar(100))'النوع',cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود العميل',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',cast(TotalAfterDiscount as nvarchar(100)) 'القيمة' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
        '        'InvoiceNo.Text = bm.SelectedRow(0)
        '        InvoiceNo_Leave(Nothing, Nothing)
        '    End If
        'End If
    ...
    Private Sub PrintPone(ByVal sender As System.Object, ByVal NewItemsOnly As Integer)
        ...
        If TestSalesAndReturn() OrElse TestPurchaseAndReturn() Then
            Select Case Md.MyProjectType
                Case ProjectType.Market, ProjectType.AboHamza
        ...
        If NewItemsOnly = 0 Then
            ...
            If Md.MyProjectType = ProjectType.PremiumMedicalSupplies Then
            ...
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.Clothes Then rpt.Rpt = "SalesPone33.rpt"
            ...
            If sender Is btnPrint2 Then
                ...
                If Md.PrintPoneA5 Then rpt.Rpt = "SalesPoneA5.rpt"

                rpt.Rpt = IIf(Md.MyProjectType = ProjectType.AboHamza, "SalesPone2_AZ.rpt", rpt.Rpt)
                rpt.Rpt = IIf(Md.MyProjectType = ProjectType.RivieraCenter, "SalesPone2_Rv.rpt", rpt.Rpt)
                ...
                If Md.MyProjectType = ProjectType.Clothes Then
                    ...
                    i = 2
                End If
                rpt.Print(".", Md.PonePrinter, i, False)
                If Md.PonePrinter2 <> "" Then rpt.Print(".", Md.PonePrinter2, i)
                Return
            End If
            ...
            If sender Is btnPrint5 Then
                ...
                ElseIf Md.MyProjectType = ProjectType.AboHamza Then
            ...
            If sender Is btnPrintSafe Then
                rpt.Print(".", Md.PonePrinter, 1, False)
                If Md.PonePrinter2 <> "" Then rpt.Print(".", Md.PonePrinter2, 1)
                Return
            End If



            If Md.MyProjectType = ProjectType.X3 AndAlso sender Is btnPrint Then
                rpt.ShowDialog()
            Else
                rpt.Show()
            End If
    ...
    Sub CalcTotalEnd() Handles Per1.TextChanged, Per2.TextChanged, Per3.TextChanged, Per4.TextChanged, Val1.LostFocus, Val2.LostFocus, Val3.LostFocus, Val4.LostFocus, AccType1.SelectionChanged, AccType2.SelectionChanged, AccType3.SelectionChanged, AccType4.SelectionChanged, Shipping.LostFocus, Freight.LostFocus, CustomClearance.LostFocus
        ...
        If Md.MyProjectType.PremiumMedicalSupplies Then
        ...
        If Md.MyProjectType.PremiumMedicalSupplies And TestSalesAndReturn() Then
    ...
    Private Sub GridKeyDown(ByVal sender As Object, ByVal e As Forms.KeyEventArgs)
        ...
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.SalesInvoiceNo).Index Then
                ...
                If bm.ShowHelpGridMultiColumns("Invoices", G.CurrentRow.Cells(GC.SalesInvoiceNo), G.CurrentRow.Cells(GC.SalesInvoiceNo), e, "select distinct M.InvoiceNo 'الفاتورة',dbo.ToStrDate(M.DayDate) 'التاريخ',cast(D.ItemId as nvarchar(100)) 'كود الصنف',D.ItemName 'اسم الصنف',cast(D.Qty as nvarchar(100)) 'الكمية',cast(D.Price -D.ItemDiscount as nvarchar(100)) 'السعر بعد الخصم',cast(D.Value as nvarchar(100)) 'القيمة' from SalesMaster M left join SalesDetails D on(M.Flag=D.Flag and M.StoreId=D.StoreId and M.InvoiceNo=D.InvoiceNo) where M.Temp=0 and M.StoreId=" & StoreId.Text & " and M.Flag=" & MainFlag() & " and (M.ToId=" & Val(ToId.Text) & " or " & Val(ToId.Text) & "=0) and (M.OrderTypeId=" & Val(OrderTypeId.Text) & " or " & Val(OrderTypeId.Text) & "=0) and (D.ItemId=" & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.Id).Value) & " or " & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.Id).Value) & "=0)") Then
                    'GridCalcRow(sender, New Forms.DataGridViewCellEventArgs(G.Columns(GC.SalesInvoiceNo).Index, G.CurrentCell.RowIndex))
            ...
            ElseIf G.CurrentCell.ColumnIndex = G.Columns(GC.Id).Index OrElse G.CurrentCell.ColumnIndex = G.Columns(GC.Name).Index Then
                Dim str As String = "select cast(Id as varchar(100)) Id," & Resources.Item("CboName") & " Name," & PriceFieldName(GC.Price, StoreUnitId) & " 'السعر' from Items where IsStopped=0 " & ItemWhere()
    ...
    Private Sub LoadItemPrice(i As Integer)
        ...
        G.Rows(i).Cells(GC.PurchasePrice).Value = Math.Round(dt.Rows(0)("PurchasePriceSub"), 2, MidpointRounding.AwayFromZero)
    ...
    Private Sub LoadVisibility()
        ...
        If Not Md.ShowTaxETA Then
            btnImportFromSalesExcel.Visibility = Visibility.Hidden
        End If
        ...
        If Md.MyProjectType = ProjectType.PremiumMedicalSupplies AndAlso (TestSalesAndReturn() OrElse TestPurchaseAndReturn()) Then
            lblCurrentBal2.Visibility = Visibility.Visible
            CurrentBal2.Visibility = Visibility.Visible
        Else
            lblCurrentBal2.Visibility = Visibility.Hidden
            CurrentBal2.Visibility = Visibility.Hidden
        End If
        ...
        If Md.MyProjectType = ProjectType.RivieraCenter AndAlso TestSalesAndReturn() Then
        ...
        ElseIf TestImportAndReturn() Then
            ...
            If Md.MyProjectType = ProjectType.X3 OrElse Md.MyProjectType = ProjectType.X4 Then
                lblDeliveryDate.Visibility = Visibility.Visible
                DeliveryDate.Visibility = Visibility.Visible
        ...
        ElseIf TestSalesAndReturn() OrElse TestExportAndReturn() Then
            ...
            If Md.MyProjectType = ProjectType.X3 OrElse Md.MyProjectType = ProjectType.X4 OrElse Md.MyProjectType = ProjectType.PremiumMedicalSupplies Then
                lblDeliveryDate.Visibility = Visibility.Visible
                DeliveryDate.Visibility = Visibility.Visible
                If Md.MyProjectType = ProjectType.X3 Then
            ...
            If Flag <> FlagState.عرض_أسعار Then 'AndAlso Flag <> FlagState.أمر_توريد 
                lblCashier.Visibility = Visibility.Visible
                CashierId.Visibility = Visibility.Visible
                CashierName.Visibility = Visibility.Visible
            ...
            If Not (Md.MyProjectType = ProjectType.MarketWithShifts OrElse Md.MyProjectType = ProjectType.Market OrElse Md.MyProjectType = ProjectType.StoresOnly OrElse Md.MyProjectType = ProjectType.X2 OrElse Md.MyProjectType = ProjectType.X3) Then
        ...
        If (Md.MyProjectType = ProjectType.RivieraCenter OrElse Md.MyProjectType = ProjectType.X1) AndAlso TestSalesOnly() AndAlso Flag <> FlagState.عرض_أسعار AndAlso Flag <> FlagState.أمر_توريد Then
            lblSalesTypeId.Visibility = Visibility.Visible
            SalesTypeId.Visibility = Visibility.Visible
        End If


        If Not Md.Manager AndAlso Not Md.MyProjectType = ProjectType.ClinicPublicElSadaka Then
        ...
        If Md.MyProjectType = ProjectType.ClinicPublicElSadaka OrElse Md.MyProjectType = ProjectType.APSD OrElse Md.MyProjectType = ProjectType.APS Then
            Payed.Visibility = Visibility.Hidden
            lblPayed.Visibility = Visibility.Hidden
            Remaining.Visibility = Visibility.Hidden
            lblRemaining.Visibility = Visibility.Hidden
        ...
        If Md.MyProjectType = ProjectType.ContractRaggal OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.أمر_شراء OrElse Flag = FlagState.عرض_أسعار_مورد Then
            lblWaiter.Visibility = Visibility.Hidden
            WaiterId.Visibility = Visibility.Hidden
            WaiterName.Visibility = Visibility.Hidden
        End If

        If (Md.MyProjectType = ProjectType.X3 OrElse Md.MyProjectType = ProjectType.X4) AndAlso Flag <> FlagState.مشتريات AndAlso Flag <> FlagState.مردودات_مشتريات AndAlso Flag <> FlagState.مشتريات_خارجية AndAlso Flag <> FlagState.مردودات_مشتريات_خارجية AndAlso Flag <> FlagState.الاستيراد AndAlso Flag <> FlagState.مردودات_الاستيراد Then
            DocNo.Visibility = Visibility.Hidden
            lblDocNo.Visibility = Visibility.Hidden
        End If
    ...
    Private Sub Hide()
        ...
        btnPoints.Visibility = Visibility.Hidden
    ...
    Private Sub G_SelectionChanged(sender As Object, e As EventArgs)
        If G.CurrentRow Is Nothing OrElse lop OrElse IsClearing Then Return
    ...
    Private Sub GetItemNameAndBal(i As Integer, Id As String, ItemSerialNo As String)
        ...
        G.Rows(i).Cells(GC.rate).Value = Val(bm.ExecuteScalar("select sum(rate) from taxableItems where ItemId=" & G.Rows(i).Cells(GC.Id).Value))
    ...
    Private Sub DocNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles DocNo.KeyUp
        If Not (Md.MyProjectType = ProjectType.X3 AndAlso TestPurchaseOnly()) Then Return
    ...
    Private Sub CalcBalSub()
        ...
        If Val(ToId.Text) > 0 Then
            If TestSalesAndReturn() Then
                ...
                dt = bm.ExecuteAdapter("exec GetSalesRPT4  {ts '2000-01-01 00:00:00'}, {ts '" & bm.MyGetDate() & " 00:00:00'}, {ts '" & bm.MyGetDate() & " 00:00:00'}, 0, 0, 0, 0, 0, 0, 31, 0, 0, " & Val(ToId.Text) & ", 0, 0, 0, 0, 0, 0, 0, 0")
    ...
    Sub GetCurrentBal()
        If Val(ToId.Text) > 0 Then
            ...
            CurrentBal2.Content = CurrentBal.Content
        Else
            CurrentBal.Content = ""
            CurrentBal2.Content = ""
        End If
    ...
    Private Sub btnImportFromSalesExcel_Click(sender As Object, e As RoutedEventArgs) Handles btnImportFromSalesExcel.Click
        Dim ExcelDT = bm.OpenExcel()
        For i As Integer = 0 To ExcelDT.Rows.Count - 1
            If ExcelDT.Rows(i)(0).ToString.Trim = "" Then Continue For
            If i = 0 Then
                btnNew_Click(Nothing, Nothing)
            ElseIf ExcelDT.Rows(i)(0).ToString.Trim <> ExcelDT.Rows(i - 1)(0).ToString.Trim OrElse ExcelDT.Rows(i)(1).ToString.Trim <> ExcelDT.Rows(i - 1)(1).ToString.Trim Then
                btnSave_Click(btnSave, Nothing)
            End If
            StoreId.Text = ExcelDT.Rows(i)(0).ToString.Trim
            DayDate.SelectedDate = ExcelDT.Rows(i)(2).ToString.Trim
            ToId.Text = AddCase(ExcelDT.Rows(i)(3).ToString.Trim)
            Dim ii As Integer = AddItem(ExcelDT.Rows(i)(4).ToString.Trim)
            G.Rows(ii).Cells(GC.Qty).Value = ExcelDT.Rows(i)(5).ToString.Trim
            G.Rows(ii).Cells(GC.Price).Value = ExcelDT.Rows(i)(6).ToString.Trim
            GridCalcRow(G, New Forms.DataGridViewCellEventArgs(G.Columns(GC.Qty).Index, ii))
        Next
        btnSave_Click(btnSave, Nothing)
    ...
    Private Sub btnPoints_Click(sender As Object, e As RoutedEventArgs) Handles btnPoints.Click
        DiscountValue.Text = bm.ExecuteScalar("select Points from Customers where Id=" & Val(ToId.Text))
    End Sub

    Sub CalcPoints()
        btnPoints.Content = "Points" & vbCrLf & "$$ " & bm.ExecuteScalar("select Points from Customers where Id=" & Val(ToId.Text)) & " $$"
    End Sub

    Private Sub ItemSearch_TextChanged(sender As Object, e As TextChangedEventArgs) Handles ItemSearch.TextChanged
        If ItemSearch.Text.Trim = "" Then Return
        For i As Integer = 0 To G.Rows.Count - 1
            If G.Rows(i).Cells(GC.Name).Value Is Nothing Then Continue For
            If G.Rows(i).Cells(GC.Name).Value.ToString.Contains(ItemSearch.Text.Trim) Then
                G.CurrentCell = G.Rows(i).Cells(GC.Name)
                Exit For
            End If
        Next
    ...
    Private Function AddCase(CaseName As String) As String
        Dim i As Integer = Val(bm.ExecuteScalar("Select Id from Customers where Name='" & CaseName & "'"))
        If i > 0 Then
            Return i
        End If


        Dim EnName As String = bm.GetEnName(CaseName)

        Dim StrAccNo As String = bm.ExecuteScalar("select top 1 Id from Chart where LinkFile=" & 1)
        bm.AddItemToTable("Customers", {"Name", "Tel", "Mobile", "Address", "AccNo", "NationalId"}, {CaseName, "", "", "", StrAccNo, ""})

        Return Val(bm.ExecuteScalar("Select Id from Cases where Name='" & CaseName & "'"))
