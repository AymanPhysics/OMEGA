...
Public Class TaxApi
    ...
    Public ClientSecret2 As String = ""

    Private DllLibPath As String = "eps2003csp11.dll"
    Private TokenPin As String = ""

    Public access_token As String = ""
    Public IsPreProd As Boolean = False
    Dim MyTable As String = "SalesMasterETA"

    Sub New(MyTokenPin As String, MyIsPreProd As Boolean)
        TokenPin = MyTokenPin
        IsPreProd = MyIsPreProd

        If IsPreProd Then
            apiBaseUrl = "https://api.preprod.invoicing.eta.gov.eg"
            idSrvBaseUrl = "https://id.preprod.eta.gov.eg"
            MyTable = "SalesMasterETAPreProd"
        End If
        Dim dt As DataTable = bm.ExecuteAdapter("select clientIdPreProd,ClientSecretPreProd,ClientSecret2PreProd,clientId,ClientSecret,ClientSecret2 from Statics")
        If dt.Rows.Count > 0 Then
            If IsPreProd Then
                clientId = dt.Rows(0)("clientIdPreProd").ToString
                ClientSecret = dt.Rows(0)("ClientSecretPreProd").ToString
                ClientSecret2 = dt.Rows(0)("ClientSecret2PreProd").ToString
            Else
    ...
    Public Sub SubmitDocuments(Flag As Integer, StoreId As Integer, InvoiceNo As Integer)
        ...
        Try
            ...
            If SalesMasterDT.Rows(0)("Exchange") Is DBNull.Value Then
                SalesMasterDT.Rows(0)("Exchange") = 1
            End If
            ...
            If CurrencySign = "" Then
                CurrencySign = "EGP"
            End If
            Dim SalesDetailsTaxDT As DataTable = bm.ExecuteAdapter("select C.TaxType,c.TaxSubType,sum(It.rate*D.Value/100)amount from SalesDetails D left join taxableItems It on (D.ItemId=It.ItemId) right join chart c on(C.Id=It.AccNo) where D.Flag=" & Flag & " and D.StoreId=" & StoreId & " and D.InvoiceNo=" & InvoiceNo & " group by C.TaxType,c.TaxSubType")

            Dim totalTaxAmount As Decimal = 0
            For Each it In SalesDetailsTaxDT.Rows
                If it("taxType").ToString <> "" Then
                    taxTotalsLines &= "{
                    ""taxType"": """ & it("taxType") & """,
                    ""amount"": " & Dec5(it("amount")) & "
                    },"
                    totalTaxAmount += Dec5(it("amount"))
                End If
            Next
            ...
            For Each it In SalesDetailsDT.Rows
                ...
                Dim taxableItemsDT As DataTable = bm.ExecuteAdapter("select *,C.TaxType,C.TaxSubType from taxableItems It left join chart c on(C.Id=It.AccNo) where ItemId=" & it("ItemId"))
                ...
                invoiceLines &= "{
                    ...
                    ""itemType"": """ & IIf(it("itemCode").ToString.StartsWith("EG"), "EGS", "GS1") & """,
                ...
                For Each itD In taxableItemsDT.Rows
                    invoiceLines &= "{
                            ""taxType"": """ & itD("TaxType") & """,
                            ""amount"": " & Dec5(it("Value") * itD("rate") / 100) & ",
                            ""subType"": """ & itD("TaxSubType") & """,
                            ""rate"": " & Dec2(itD("rate")) & "
                        },"
                Next
            ...
            ""internalID"": """ & SalesMasterDT.Rows(0)("StoreId") & "_" & SalesMasterDT.Rows(0)("Flag") & "_" & SalesMasterDT.Rows(0)("InvoiceNo") & """,
            ...
            ""totalAmount"": " & Dec5((SalesMasterDT.Rows(0)("Total") + totalTaxAmount) * IIf(SalesMasterDT.Rows(0)("CurrencyId") = 1, 1, SalesMasterDT.Rows(0)("Exchange"))) & ",
            ...
            If result0("acceptedDocuments").Count > 0 Then
                ...
                bm.ExecuteNonQuery("delete " & MyTable & " where StoreId=" & SalesMasterDT.Rows(0)("StoreId") & " and Flag=" & SalesMasterDT.Rows(0)("Flag") & " and InvoiceNo=" & SalesMasterDT.Rows(0)("InvoiceNo") & "    insert " & MyTable & " (StoreId,Flag,InvoiceNo,uuid,longId) select " & SalesMasterDT.Rows(0)("StoreId") & "," & SalesMasterDT.Rows(0)("Flag") & "," & SalesMasterDT.Rows(0)("InvoiceNo") & ",'" & result0("acceptedDocuments")(0)("uuid") & "','" & result0("acceptedDocuments")(0)("longId") & "'")
            End If
            If result0("rejectedDocuments").Count > 0 Then
                ...
                Next
                bm.ExecuteNonQuery("delete " & MyTable & " where StoreId=" & SalesMasterDT.Rows(0)("StoreId") & " and Flag=" & SalesMasterDT.Rows(0)("Flag") & " and InvoiceNo=" & SalesMasterDT.Rows(0)("InvoiceNo") & "    insert " & MyTable & " (StoreId,Flag,InvoiceNo,Notes) select " & SalesMasterDT.Rows(0)("StoreId") & "," & SalesMasterDT.Rows(0)("Flag") & "," & SalesMasterDT.Rows(0)("InvoiceNo") & ",' " & msg & "'")
            End If
        Catch ex As Exception
            ...
            If DocumentDetails <> "Valid" AndAlso DocumentDetails <> "Submitted" Then
                For Each r In JsonConvert.DeserializeObject(result)("validationResults")("validationSteps")
                    If r("status") = "Invalid" Then
                        ...
                        Next
                        bm.ExecuteNonQuery("update " & MyTable & " set NeedReSubmit=1 where uuid='" & DocumentUniqueID & "'")
                    End If
                Next
            End If
            bm.ExecuteNonQuery("update " & MyTable & " set DocumentDetails='" & DocumentDetails & "' where uuid='" & DocumentUniqueID & "'")
        Catch ex As Exception
            bm.ShowMSG(ex.Message)
        End Try
    End Sub
